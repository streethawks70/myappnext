...
import type { Posizione } from "@/components/MappaLeaflet2";
import Link from 'next/link'; 
import DistrettoSelector from "@/components/DistrettoSelector";

const MappaLeafletComponent1 = dynamic(() => import("@/components/MappaLeaflet2"), { ssr: false });

export default function MappaPage1() {
  ...
  useEffect(() => {
    ...
    const caricaDati = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/dato?email=${email}&password=${password}&distretto=${distretto}`);
...
{email && password && distretto && (
  <div className="mb-4">
    <Link
      href={`/replay?email=${email}&password=${password}&comparto=${distretto}`}
    ...
    >
      ...
      <div className="h-[600px] w-full border rounded overflow-hidden">
        <MappaLeafletComponent1 posizioni={posizioniFiltrate}
